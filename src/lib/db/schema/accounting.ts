...
// =============================================================================
// 22. ASSET CATEGORIES — Categorias de ativos
// =============================================================================
export const assetCategories = pgTable(
  "asset_categories",
  {
    ...
    description: text("description"),
    depreciationMethod: varchar("depreciation_method", { length: 50 }).default("straight_line").notNull(), // straight_line, declining_balance, double_declining, units_of_production
    ...
    expenseAccount: uuid("expense_account").references(() => chartOfAccounts.id),
    accumulatedDepreciationAccount: uuid("accumulated_depreciation_account").references(() => chartOfAccounts.id),
    disposalAccount: uuid("disposal_account").references(() => chartOfAccounts.id),
    insuranceAccount: uuid("insurance_account").references(() => chartOfAccounts.id),
    maintenanceExpenseAccount: uuid("maintenance_expense_account").references(() => chartOfAccounts.id),
    capitalWorkInProgressAccount: uuid("capital_work_in_progress_account").references(() => chartOfAccounts.id),
    isActive: boolean("is_active").default(true).notNull(),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").defaultNow().notNull(),
  },
...
// =============================================================================
// 23. ASSETS — Ativos fixos COMPLETO
// =============================================================================
export const assets = pgTable(
  "assets",
  {
    ...
    description: text("description"),
    assetStatus: varchar("asset_status", { length: 50 }).default("active").notNull(), // in_use, in_maintenance, scrapped, sold, draft
    ...
    salvageValue: decimal("salvage_value", { precision: 20, scale: 4 }).default("0"),
    ...
    isOpeningEntry: boolean("is_opening_entry").default(false).notNull(),
    disposeDate: date("dispose_date"),
    disposalAmount: decimal("disposal_amount", { precision: 20, scale: 4 }),
    notes: text("notes"),
    isActive: boolean("is_active").default(true).notNull(),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").defaultNow().notNull(),
...
// =============================================================================
// 24. DEPRECIATION ENTRIES — Entradas de depreciação
// =============================================================================
export const depreciationEntries = pgTable(
  "depreciation_entries",
  {
    ...
    journalEntryId: uuid("journal_entry_id").references(() => journalEntries.id),
    status: varchar("status", { length: 20 }).default("pending").notNull(), // pending, posted, cancelled
  ...
  (t) => [
    ...
    index("depreciation_entries_fiscal_year_idx").on(t.fiscalYearId),
  ]
);
...
// =============================================================================
// 34. DUNNING LETTERS — Cartas de cobrança
// =============================================================================
export const dunningLetters = pgTable(
  "dunning_letters",
  {
    id: uuid("id").defaultRandom().primaryKey(),
    tenantId: uuid("tenant_id").notNull(),
    companyId: uuid("company_id")
      .notNull()
      .references(() => companies.id, { onDelete: "cascade" }),
    dunningTypeId: uuid("dunning_type_id")
      .references(() => dunningTypes.id),
    partyType: varchar("party_type", { length: 50 }).notNull(),
    partyId: uuid("party_id").notNull(),
    ...
    feeAmount: decimal("fee_amount", { precision: 20, scale: 4 }).default("0"),
    totalDunningAmount: decimal("total_dunning_amount", { precision: 20, scale: 4 }).default("0"),
    letterSubject: varchar("letter_subject", { length: 500 }),
    letterBody: text("letter_body"),
    sentDate: date("sent_date"),
    sentVia: varchar("sent_via", { length: 50 }), // email, post, sms
    status: varchar("status", { length: 20 }).default("draft").notNull(), // draft, sent, responded, closed
  ...
  (t) => [
    index("dunning_letters_tenant_idx").on(t.tenantId),
    index("dunning_letters_company_idx").on(t.companyId),
    index("dunning_letters_type_idx").on(t.dunningTypeId),
    index("dunning_letters_party_idx").on(t.partyType, t.partyId),
    index("dunning_letters_date_idx").on(t.dunningDate),
    index("dunning_letters_status_idx").on(t.status),
  ]
);

// =============================================================================
// 35. SUBSCRIPTION PLANS — Planos de assinatura
// =============================================================================
export const subscriptionPlans = pgTable(
  "subscription_plans",
  {
    ...
    billingInterval: varchar("billing_interval", { length: 50 }).notNull(), // daily, weekly, monthly, quarterly, yearly
    billingFrequency: integer("billing_frequency").default(1),
    price: decimal("price", { precision: 20, scale: 4 }).notNull(),
    currency: varchar("currency", { length: 3 }).default("USD"),
    setupFee: decimal("setup_fee", { precision: 20, scale: 4 }).default("0"),
    trialPeriodDays: integer("trial_period_days").default(0),
    accountId: uuid("account_id").references(() => chartOfAccounts.id),
    revenueAccount: uuid("revenue_account").references(() => chartOfAccounts.id),
    description: text("description"),
    isActive: boolean("is_active").default(true).notNull(),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").defaultNow().notNull(),
  ...
  (t) => [
    ...
    index("subscription_plans_company_idx").on(t.companyId),
  ]
);
...
export const subscriptions = pgTable(
  "subscriptions",
  {
    ...
    amount: decimal("amount", { precision: 20, scale: 4 }).notNull(),
    currency: varchar("currency", { length: 3 }).default("USD"),
    status: varchar("status", { length: 50 }).default("active"), // active, trial, cancelled, past_due, unpaid, paused
    paymentFailedCount: integer("payment_failed_count").default(0),
    autoRenew: boolean("auto_renew").default(true).notNull(),
    salesInvoiceId: uuid("sales_invoice_id").references(() => invoices.id),
    costCenterId: uuid("cost_center_id").references(() => costCenters.id),
    remarks: text("remarks"),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").defaultNow().notNull(),
...
// =============================================================================
// 37. SUBSCRIPTION INVOICES — Faturas de assinatura
// =============================================================================
export const subscriptionInvoices = pgTable(
  "subscription_invoices",
  {
    ...
    paymentRetryDate: date("payment_retry_date"),
    remarks: text("remarks"),
    createdAt: timestamp("created_at").defaultNow().notNull(),
    updatedAt: timestamp("updated_at").defaultNow().notNull(),
  },
  (t) => [
    ...
    index("subscription_invoices_invoice_idx").on(t.invoiceId),
    index("subscription_invoices_date_idx").on(t.invoiceDate),
