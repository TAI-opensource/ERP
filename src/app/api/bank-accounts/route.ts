...
export async function POST(request: NextRequest) {
  try {
    ...
    const { tenantId, companyId, accountCode, accountName, currency, description } = body;

    if (!tenantId || !companyId || !accountCode || !accountName) {
      return badRequest("tenantId, companyId, accountCode, and accountName are required");
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!updated) return badRequest("Bank account not found");

    return success(updated);
  } catch (error) {
    console.error("PUT /api/bank-accounts error:", error);
    return serverError();
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!deleted) return badRequest("Bank account not found");
