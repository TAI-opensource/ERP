...
type JournalEntryStatus = "draft" | "submitted" | "cancelled" | "voided"
...
const statusConfig: Record<JournalEntryStatus, { label: string; color: string; bgColor: string }> = {
  draft: { label: "Draft", color: "text-gray-700 dark:text-gray-200", bgColor: "bg-gray-100 dark:bg-gray-800" },
  submitted: { label: "Submitted", color: "text-green-700 dark:text-green-200", bgColor: "bg-green-100 dark:bg-green-900/50" },
  cancelled: { label: "Cancelled", color: "text-red-700 dark:text-red-200", bgColor: "bg-red-100 dark:bg-red-900/50" },
  voided: { label: "Voided", color: "text-orange-700 dark:text-orange-200", bgColor: "bg-orange-100 dark:bg-orange-900/50" },
}
  ...
  const filteredEntries = React.useMemo(() => {
    ...
    if (searchQuery) {
      ...
      result = result.filter(
        (e) =>
          e.voucherNumber.toLowerCase().includes(lower) ||
    ...
    if (typeFilter !== "all") {
      result = result.filter((e) => e.voucherType === typeFilter)
    }
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Select value={typeFilter} onValueChange={(v) => setTypeFilter(v ?? "all")}>
            ...
            <SelectContent>
              ...
              <SelectItem value="opening_entry">Opening Entry</SelectItem>
              <SelectItem value="closing_entry">Closing Entry</SelectItem>
            </SelectContent>
          </Select>
        </div>
                ...
                paginatedEntries.map((entry) => {
                  ...
                  return (
                    <TableRow key={entry.id}>
                      ...
                      <TableCell className="text-right font-mono text-sm tabular-nums">
                        {entry.totalCredit.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                      </TableCell>
                      <TableCell>
                        <span
                          className={cn(
